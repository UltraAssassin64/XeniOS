...
bool RegionUnlockWrite(void* address, size_t length) {
  // FIX (iOS 16 TrollStore JIT): before flipping to RW, widen the Mach
  // max protections to include execute. On iOS 16 without TXM, mprotect()
  // to re-add PROT_EXEC later (in RegionSetExec) is rejected if execute
  // was never in the max protection set. vm_protect(TRUE, RWX) widens the
  // max set without changing current protections, satisfying the kernel
  // check when we later call mprotect() to restore RX.
  uintptr_t aligned_start = 0;
  size_t aligned_length = 0;
  if (GetPageAlignedRange(address, length, aligned_start, aligned_length) &&
      aligned_length) {
...
bool RegionSetExec(void* address, size_t length) {
  // FIX (iOS 16 TrollStore JIT): widen max protections to RWX before the
  // mprotect call that restores RX. On iOS 16 without TXM the kernel rejects
  // mprotect(PROT_EXEC) unless execute is already in the mapping's Mach max
  // protections. The vm_protect(TRUE) call here is a no-op if the max was
  // already widened by RegionUnlockWrite or the initial allocation.
  uintptr_t aligned_start = 0;
  size_t aligned_length = 0;
  if (GetPageAlignedRange(address, length, aligned_start, aligned_length) &&
      aligned_length) {
...
bool A64CodeCache::Initialize() {
  ...
  if (xe::memory::IsWritableExecutableMemoryPreferred()) {
    ...
    if (!generated_code_execute_base_ || !generated_code_write_base_) {
      ...
      // FIX (iOS 16 TrollStore JIT): map initially as PROT_READ|PROT_EXEC so
      // that EXECUTE is present in the Mach mapping's max protections from the
      // start. On iOS 16 without TXM, mprotect() to add PROT_EXEC on a mapping
      // that was created PROT_READ|PROT_WRITE is rejected by the kernel because
      // execute was never in the max protections. Starting from RX and then
      // immediately flipping to RW keeps execute in the max set permanently,
      // allowing the per-function RW→RX transitions in PlaceGuestCode to
      // succeed via vm_protect(TRUE) + mprotect().
      generated_code_write_base_ = reinterpret_cast<uint8_t*>(
          mmap(nullptr, kGeneratedCodeSize, PROT_READ | PROT_EXEC,
               MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
      if (generated_code_write_base_ == MAP_FAILED) {
        generated_code_write_base_ = nullptr;
        XELOGE("Unable to allocate iOS JIT code cache (initial RX mapping)");
        return false;
      }
      // Widen Mach max protections to include RWX before flipping to RW.
      // This is the critical step: without it the later per-function
      // vm_protect(TRUE, RWX) calls in RegionUnlockWrite/RegionSetExec fail.
      {
        const kern_return_t kr_max = vm_protect(
            mach_task_self(),
            reinterpret_cast<vm_address_t>(generated_code_write_base_),
            kGeneratedCodeSize, TRUE,
            VM_PROT_READ | VM_PROT_WRITE | VM_PROT_EXECUTE);
        if (kr_max != KERN_SUCCESS) {
          XELOGW(
              "iOS JIT mprotect-flip: vm_protect set-max RWX failed (kr={}) "
              "— per-function RX transitions may fail on iOS 16",
              kr_max);
        }
      }
      // Now flip to RW so the cache is writable from the start. Individual
      // PlaceGuestCode calls will flip pages RW→RX per function.
      if (mprotect(generated_code_write_base_, kGeneratedCodeSize,
                   PROT_READ | PROT_WRITE) != 0) {
        XELOGE(
            "iOS JIT mprotect-flip: initial RW flip failed errno={} ({})",
...
void A64CodeCache::PlaceGuestCode(uint32_t guest_address, void* machine_code,
  ...
  UnwindReservation unwind_reservation;
  {
    ...
    //
    // FIX (out-of-bounds PlaceGuestCode): guard against the cache being full.
    // The log showed faults at cache_base + kGeneratedCodeSize (one byte past
    // the end) because the offset was allowed to grow to exactly kGeneratedCodeSize,
    // placing a function with zero bytes of usable cache remaining. Any attempt
    // to execute instructions at that address immediately faults.
    //
    // Check that both the code and unwind reservation fit before committing the
    // offset bump. If they don't, log a fatal error and return null pointers —
    // the caller (A64Assembler) checks for null and will propagate the failure
    // gracefully rather than crashing the process.
    {
      const size_t code_rounded = xe::round_up(func_info.code_size.total, 16);
      // Estimate unwind reservation size conservatively (matches the reservation
      // made below). We use kMaximumFunctionCount * 0 since data_size isn't
      // known yet; use a safe upper bound of 256 bytes (kMaxUnwindInfoSize
      // rounded to 16).
      constexpr size_t kUnwindUpperBound = 256;
      const size_t total_needed = code_rounded + kUnwindUpperBound;
      if (generated_code_offset_ + total_needed > kGeneratedCodeSize) {
        XELOGE(
            "PlaceGuestCode: code cache FULL — offset=0x{:X} needed=0x{:X} "
            "limit=0x{:X}; guest=0x{:08X} code_size={}",
            generated_code_offset_, total_needed,
            static_cast<size_t>(kGeneratedCodeSize), guest_address,
...
#if XE_PLATFORM_IOS && XE_ARCH_ARM64
    if (generated_code_uses_mprotect_flip_) {
      // FIX: replaced assert_always() with a safe log + null-return so that
      // a failed mprotect does not crash the whole process. The caller checks
      // for null output pointers and propagates the error appropriately.
      if (!RegionLockRead(code_write_address, write_span_length)) {
        XELOGE(
            "iOS JIT mprotect flip: RegionLockRead failed "
            "addr={:p} len=0x{:X} guest=0x{:08X}",
            static_cast<void*>(code_write_address), write_span_length,
            guest_address);
        code_execute_address_out = nullptr;
        code_write_address_out   = nullptr;
        return;
      ...
      if (!RegionUnlockWrite(code_write_address, write_span_length)) {
        XELOGE(
            "iOS JIT mprotect flip: RegionUnlockWrite failed "
            "addr={:p} len=0x{:X} guest=0x{:08X}",
            static_cast<void*>(code_write_address), write_span_length,
            guest_address);
        code_execute_address_out = nullptr;
    ...
    if (generated_code_uses_mprotect_flip_) {
      ...
      // iOS 18 and below.
      // FIX: replaced assert_always() — a failed flip is logged and the
      // function returns with null pointers instead of crashing the process.
      if (!RegionSetExec(code_execute_address, write_span_length)) {
        XELOGE(
            "iOS JIT mprotect flip: RegionSetExec failed after code write "
            "addr={:p} len=0x{:X} guest=0x{:08X}",
            static_cast<void*>(code_execute_address), write_span_length,
            guest_address);
        code_execute_address_out = nullptr;
        code_write_address_out   = nullptr;
        return;
